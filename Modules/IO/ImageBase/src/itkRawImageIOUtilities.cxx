                                   ...
                                   itk::SizeValueType numberOfComponents)
{
  ...
  if (byteOrder == itk::ByteOrderEnum::LittleEndian && InternalByteSwapperType::SystemIsBigEndian())
  {
  ...
  else if (byteOrder == itk::ByteOrderEnum::BigEndian && InternalByteSwapperType::SystemIsLittleEndian())
  {
    TStrongType * tempBuffer = new TStrongType[numberOfPixels];
    memcpy((char *)tempBuffer, buffer, numberOfBytes);
...
template <typename TStrongType>
void
_ReadRawBytesAfterSwappingUtility(void * buffer, itk::ByteOrderEnum byteOrder, itk::SizeValueType numberOfComponents)
{
  using InternalByteSwapperType = itk::ByteSwapper<TStrongType>;
  if (byteOrder == itk::ByteOrderEnum::LittleEndian)
  {
    InternalByteSwapperType::SwapRangeFromSystemToLittleEndian((TStrongType *)buffer, numberOfComponents);
  }
  else if (byteOrder == itk::ByteOrderEnum::BigEndian)
  {
